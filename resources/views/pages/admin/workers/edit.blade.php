...
@section('admin-content')
    <div class="container mx-auto px-4 py-8">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold">Modifier le salarié: {{ $worker->last_name }} {{ $worker->first_name }}</h1>
        ...
        <div class="bg-white shadow-md rounded-lg p-6">
            <form method="POST" action="{{ route('admin.workers.update', $worker) }}">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="last_name" id="last_name" value="{{ old('last_name', $worker->last_name) }}"
                ...
                <div class="mb-4">
                    ...
                    <select name="category" id="category"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                        required>
                        <option value="worker" {{ old('category', $worker->category) === 'worker' ? 'selected' : '' }}>
                            Ouvrier</option>
                        <option value="etam" {{ old('category', $worker->category) === 'etam' ? 'selected' : '' }}>ETAM
                        </option>
                    </select>
                    @error('category')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                ...
                <div class="mb-4">
                    <label for="contract_hours" class="block text-sm font-medium text-gray-700">Heures de contrat
                        (hebdomadaires)</label>
                    <input type="number" name="contract_hours" id="contract_hours"
                        value="{{ old('contract_hours', $worker->contract_hours) }}"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                        required>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" step="0.01" name="monthly_salary" id="monthly_salary"
                        value="{{ old('monthly_salary', $worker->monthly_salary) }}"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                        required>
                ...
                <div class="mb-4">
                    ...
                    <select name="status" id="status"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                        required>
                        <option value="active" {{ old('status', $worker->status) === 'active' ? 'selected' : '' }}>Actif
                        </option>
