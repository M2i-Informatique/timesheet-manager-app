...
@section('title', 'Modifier un intérimaire')

@section('admin-content')
    <div class="container mx-auto px-4 py-8">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold">Modifier l'intérimaire: {{ $interim->agency }}</h1>
        ...
        <div class="bg-white shadow-md rounded-lg p-6">
            <form method="POST" action="{{ route('admin.interims.update', $interim) }}">
                @csrf
                @method('PUT')

                <div class="mb-4">
                    <label for="agency" class="block text-sm font-medium text-gray-700">Agence</label>
                    <input type="text" name="agency" id="agency" value="{{ old('agency', $interim->agency) }}"
                ...
                <div class="mb-4">
                    ...
                    <input type="number" step="0.01" name="hourly_rate" id="hourly_rate"
                        value="{{ old('hourly_rate', $interim->hourly_rate) }}"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                        required>
                    @error('hourly_rate')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                ...
                <div class="mb-4">
                    ...
                    <select name="status" id="status"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                        required>
                        <option value="active" {{ old('status', $interim->status) === 'active' ? 'selected' : '' }}>Actif
                        </option>
                        <option value="inactive" {{ old('status', $interim->status) === 'inactive' ? 'selected' : '' }}>
                            Inactif</option>
                    </select>
                    @error('status')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
